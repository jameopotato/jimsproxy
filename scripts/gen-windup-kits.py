...
Rule:   keep rows with StartEvent=3, EndEvent=13, TargetType=1 (a caster-side wind-up that closes with the
        cast), then drop any kit that also appears under a different (StartEvent, EndEvent, TargetType),
        so that a cancel by kit id (SMSG_CANCEL_SPELL_VISUAL_KIT) can never hit a non-wind-up effect.
...
def main(src: str, dst: str) -> None:
    ...
    windup = {(int(r["SpellVisualID"]), int(r["SpellVisualKitID"]))
              for r in rows if r["StartEvent"] == "3" and r["EndEvent"] == "13" and r["TargetType"] == "1"}
    windup_kits = {k for _, k in windup}
    dual_use = {int(r["SpellVisualKitID"]) for r in rows
                if int(r["SpellVisualKitID"]) in windup_kits
                and not (r["StartEvent"] == "3" and r["EndEvent"] == "13" and r["TargetType"] == "1")}
