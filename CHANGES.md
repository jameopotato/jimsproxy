...
## 2026-08-03 — World-entry root-ceremony breadcrumb (always-on) + dev delivery harness (stage 0.5)

**Issue:** The 2026-08-02 stage-0 captures (12 BG transitions on the Kronos dev's local core + 4 PTR transfers) established that every arrival delivers a fixed force-op ceremony — SMSG_MOVE_ROOT ×2 at the loading boundary, SMSG_MOVE_UNROOT ~1s later, every leg client-acked, every MoveCounter 0 — and that a root whose unroot leg fails is the leading mechanism for the post-BG-exit movement lockup (translation dead, turning/casting alive, `/reload` cures; the symptom is root semantics). Two gaps: (1) the ceremony's health was only visible under DebugOutput, so a reporter's normal Export Diagnostics could never carry the discriminator; (2) forcing the suspected race required server-side changes or a slow machine losing it naturally.

**Change:** Two independent pieces, both centered on `World/Client/WorldEntryCeremony.cs` (logic extracted for tests, TransportClearGate precedent). **(a) Always-on breadcrumb:** `GameSessionData.WorldEntryCeremony` counts the player's ROOT/UNROOT forwards (Client/MovementHandler `HandleMoveForceFlagChange`) and their acks (Server/MovementHandler `HandleMoveForceAck2`) between arrival anchors (NEW_WORLD forward / login verify), stamps `CMSG_MOVE_INIT_ACTIVE_MOVER_COMPLETE`, and at the next boundary logs ONE `worldentry.ceremony.unclosed` line if the ceremony opened a root without observably closing it (no unroot = server skip; unroot unacked = client swallow — the stuck-stun golden capture's discard fingerprint; root acks short = a root leg discarded). Healthy ceremonies log only under DebugOutput (`.closed`). Packet-ordered, no timers, at most one line per transition. **(b) Dev harness (config-gated, default off, launcher never sets them):** `WorldEntryHarnessMode=drop_unroot` swallows the next N arrival unroots (symptom-validation experiment); `hold_until_init` queues arrival ROOT/UNROOT while the client is loading and releases at init-complete + `WorldEntryHarnessDelayMs` (fail-open: leftovers flush at the next boundary); `WorldEntryMintMoveCounters` replaces the constant-zero force-op counters with session-monotonic values toward the client and restores the original on the ack path (A/B lever for the counter-collision theory).

**Verification:** Build clean; suite 790/790 (8 new: unclosed-predicate truth table incl. the golden-capture fingerprint shapes, tracker begin/reset, mint monotonic/resolve-once/unknown-passthrough). Zero-behavior-change claim: with defaults, the only new runtime effects are counter increments and at most one JSONL line per transition. Field gate for the breadcrumb: the 2026-08-02 healthy captures replayed through this build must produce zero `unclosed` lines; the harness's `drop_unroot` run must produce exactly the reported symptom table including the `/reload` cure — that result is the mechanism confirmation, not this PR.
